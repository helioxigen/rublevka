...
class AutoAuthFlaskClient(FlaskClient):
    ...
    def login(self, user, auth_scopes=None):
        """
        Example:
            >>> with client.login(user, auth_scopes=['users:read']):
    ...
    def open(self, *args, **kwargs):
        if self._user is not None:
            from jqestate.extensions import db
            from jqestate.modules.auth.models import OAuth2Client, OAuth2Token
...
def generate_user_instance(
    ...
    from jqestate.modules.users.models import User
    if password is None:
