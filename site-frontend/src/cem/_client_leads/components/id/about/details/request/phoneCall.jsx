...
class About extends Component {
  changeKind(requestKind) {
    ...
    this.props.actions.updateLead(data.id, lead).then(
  ...
  render() {
    ...
    const isStatic = (formKey !== `create` && !isUpdateAllowed) || [`spam`, `rejected`, `processed`].indexOf(state) > -1;

    return (
      <section className={s.section}>
        <CallDetails phoneCallDetails={data.phoneCallDetails} />
        {(formKey === `create` || isSensitiveDataVisible) && <ContactDetails fields={this.props.fields.contactDetails} isStatic={isStatic} />}
            ...
            <Col md="18">
              <Row sm="center">
                ...
                <Col sm="6" smOffset="1" className={sUtils.flexWrap}>
                  ...
                  <div className={s.flexItem}>
                    {state === `new` && (
                      <Button size="xs" kind="warning" onClick={() => ::this.changeKind(`selection`)}>выбрать тип заявки</Button>
                    )}
                  </div>
                </Col>
                ...
                <Col sm="6" smOffset="1" className={sUtils.flexWrap}>
                  ...
                  <div className={s.flexItem}>
                    {state === `new` && (
                      <Button size="xs" kind="warning" onClick={() => ::this.changeKind(`selling`)}>выбрать тип заявки</Button>
                    )}
                  </div>
                </Col>
              </Row>
