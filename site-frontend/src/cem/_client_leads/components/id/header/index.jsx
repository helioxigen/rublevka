...
class Header extends Component {
  ...
  render() {
    const {
      data = {}, requestKind, id, state, actions,
      formKey, handleSubmit, pristine, error, submitting,
      isUpdateAllowed,
    } = this.props;
    ...
    const canTransfer = (leadState === `new` || leadState === `in_progress`) && !stateDetails.toApprove;
        ...
        <Form.Container onSubmit={handleSubmit(::this.createOrUpdate, ::this.onSubmitSuccess)}>
          <Grid.Container fluid>
              ...
              <Col xs="20" className={sUtils.positionRelative}>
                ...
                {canTransfer &&
                  <Dropdown className={sDropdown.header} button={<Button type="button" className={sButton.btnDropdown}><Icon className={s.iconSubmenu} icon="submenu" /></Button>}>
                    <TransferUserModal className={sUtils.displayBlock} objectKind="client_leads" objectId={data.id} destinationKind="users" responsibleUser={responsibleUser.data} reloadAction={actions.loadLead}>
                      <Button type="button" className={sButton.btnDropdownInner}>Передать другому сотруднику</Button>
                    </TransferUserModal>
                    <TransferUserModal className={sUtils.displayBlock} objectKind="client_leads" objectId={data.id} destinationKind="departments" responsibleUser={responsibleUser.data} reloadAction={actions.loadLead}>
            ...
            <div className={sUtils.extraPadding}>
              <LeadDetails {...this.props}
                state={leadState} stateToApprove={stateDetails.toApprove}
              />
              ...
              {id !== `create` && (
                <ManagerControls
                  state={leadState} stateDetails={stateDetails}
                  isCurrentUserDepartmentManager={state.auth.details.isManager && !state.auth.details.divisionId}
                  process={::this.process}
