              ...
              <Row>
                <Cell>
                  <Group className={sUtils.resetIndentation} kind={fields.kindId.touched && !!fields.kindId.error && `error`}>
                    <AsyncSelect className={sUtils.resetBorder} asyncOptions={fetchDictionary(`contact_document_type`)} {...fields.kindId} labelKey="title" valueKey="id" onBlur={() => {}} />
                ...
                <Cell>
                  <Group className={sUtils.resetIndentation}>
                    <Input className={s.tableInput} type="text" placeholder="Добавить комментарий" {...fields.comment} />
                  </Group>
                </Cell>
                {formKey !== `add` && (
                  <Cell>
                    <Group className={sUtils.resetIndentation}>
                      ...
                      <Button className={sButton.btnTableAction} size="xs" to={`${API}/v1/contacts/${this.props.contactId}/documents/${formKey}/download?token=${state.auth.token}`} target="_blank">
...
class Documents extends Component {
  ...
  render() {
    const { actions, state, state: { documents: { items = [] } } } = this.props;
    ...
    return (
      <div className={sUtils.scrollX}>
        <Table.Container width="100%" className={sUtils.width120}>
          ...
          {items.map((item, index) => <DocumentForm key={index} contactId={this.props.contactId} formKey={item.id.toString()} actions={actions} initialValues={item} state={state} />)}
        </Table.Container>
      </div>
    );
  }
...
export default connect(pickState, pickActions)(Documents);
