...
const update = (id: number | string, values) => (dispatch) => {
  return new Promise((resolve, reject) => {
    ...
    return updateElement(apiPath, id, recursiveCleanUp(transformOutputValues(values)))
      .then(response => {
        dispatch(updateElementSucceeded(types.UPDATE_SUCCEEDED, id, transformInputValues(response)));
