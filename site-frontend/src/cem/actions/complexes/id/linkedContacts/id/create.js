...
export default (complexData, data) => (dispatch) => {
  ...
  return API.post('/v1/contacts', { ...data, additionalDetails: { autoRegion: undefined }, companyDetails: {} }).then(
    ({ headers }) => API.get(headers.location).then(({ body }) => {
      dispatch(updateComplex(complexData.id, { ...complexData, linkedContactIds: [...complexData.linkedContactIds, body.id] }))
      ...
      if (errors && errors.some(error => error.param === 'details.phoneNumber' || error.param === 'phoneNumber')) {
        return API.get('/v1/contacts', { filter: { 'details.phoneNumber': normalizePhoneNumber(data.details.phoneNumber) } }).then(
          ({ body }) => {
            ...
            dispatch(updateComplex(complexData.id, { ...complexData, linkedContactIds: [...complexData.linkedContactIds, existingContactData.id] }))
              .then(() => dispatch(createContactSucceeded(body)));
