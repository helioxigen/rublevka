...
export const transformProperty = (property, category) => {
  ...
  if (values.complexBuildingDetails) {
    values.complexBuildingDetails = transformComplexBuilding(
      values.complexBuildingDetails,
    );
  }
  if (values.saleOffer) {
    if (
      values.saleOffer.isAgentFixed === 'true' &&
      values.saleOffer.agentFixedPrice
    ) {
  ...
  if (values.rentOffer) {
    if (
      values.rentOffer.isAgentFixed === 'true' &&
      values.rentOffer.agentFixedPrice
    ) {
    ...
    values.rentOffer.isAllowedChildren =
      values.rentOffer.isAllowedChildren === 'true';
  ...
  if (values.category === 'city') {
    ...
    values.specification.balconies = normalizeNumber(
      values.specification.balconies,
    );
    values.specification.loggias = normalizeNumber(
      values.specification.loggias,
    );

    values.specification.ceilHeight = normalizeNumber(
      values.specification.ceilHeight,
    );
    values.specification.floor = normalizeNumber(values.specification.floor);
    values.specification.livingArea = normalizeNumber(
      values.specification.livingArea,
    );
    values.specification.totalArea = normalizeNumber(
      values.specification.totalArea,
    );
    ...
    values.specification.bedrooms = normalizeNumber(
      values.specification.bedrooms,
    );

    values.specification.panoramicGlazing =
      values.specification.panoramicGlazing === 'true';
  }

  // Country-specific transforms
  if (values.category === 'country') {
    values.specification.withConditioning =
      values.specification.withConditioning === 'true';
    values.specification.withVentilation =
      values.specification.withVentilation === 'true';

    values.specification.bedrooms = normalizeNumber(
      values.specification.bedrooms,
    );
    values.specification.area = normalizeNumber(values.specification.area);
    values.specification.builtYear = normalizeNumber(
      values.specification.builtYear,
    );
    ...
    values.specification.elevators = normalizeNumber(
      values.specification.elevators,
    );
    values.specification.ceilingHeight = normalizeNumber(
      values.specification.ceilingHeight,
    );
    ...
    values.additionalDetails.spaArea = normalizeNumber(
      values.additionalDetails.spaArea,
    );
    ...
    values.additionalDetails.parkingArea = normalizeNumber(
      values.additionalDetails.parkingArea,
    );
    values.additionalDetails.garageArea = normalizeNumber(
      values.additionalDetails.garageArea,
    );
    ...
    values.communication.powerSupply = normalizeNumber(
      values.communication.powerSupply,
    );

    values.specification.legacyLayouts =
      values.specification.legacyLayouts || [];
...
export const transformIn = property => {
  ...
  if (values.saleOffer) {
    values.saleOffer.isAgentFixed = String(
      !!values.saleOffer.agentFixedPrice &&
        !!Object.keys(values.saleOffer.agentFixedPrice).length,
    );
    values.saleOffer.agentFee = normalizeNumber(values.saleOffer.agentFee);

    values.saleOffer.isBargain = values.saleOffer.isBargain ? 'true' : 'false';
    values.saleOffer.isInstallment = values.saleOffer.isInstallment
      ? 'true'
      : 'false';
    values.saleOffer.isMortgage = values.saleOffer.isMortgage
      ? 'true'
      : 'false';
    values.saleOffer.isDisabled = values.saleOffer.isDisabled
      ? 'true'
      : 'false';
  ...
  if (values.rentOffer) {
    values.rentOffer.isAgentFixed = String(
      !!values.rentOffer.agentFixedPrice &&
        !!Object.keys(values.rentOffer.agentFixedPrice).length,
    );
    values.rentOffer.agentFee = normalizeNumber(values.rentOffer.agentFee);

    values.rentOffer.isAllowedChildren = values.rentOffer.isAllowedChildren
      ? 'true'
      : 'false';
    values.rentOffer.isAllowedPets = values.rentOffer.isAllowedPets
      ? 'true'
      : 'false';
    values.rentOffer.isDisabled = values.rentOffer.isDisabled
      ? 'true'
      : 'false';
  }
  ...
  if (values.category === 'country') {
    if (values.specification) {
      values.specification.withConditioning = values.specification
        .withConditioning
        ? 'true'
        : 'false';
      values.specification.withVentilation = values.specification
        .withVentilation
        ? 'true'
        : 'false';
    }
    ...
    if (values.landDetails) {
      values.landDetails.landscaping = values.landDetails.landscaping
        ? 'true'
        : 'false';
    }
  } else if (values.category === 'city') {
    if (values.specification) {
      values.specification.panoramicGlazing = values.specification
        .panoramicGlazing
        ? 'true'
        : 'false';
    }
