...
const createPropertyFailed = errors => dispatch => dispatch({
    ...
    if (Object.keys(recursiveCleanUp(residentialComplex || {})).length) {
      if (!data.residentialComplexId) {
        return dispatch(createResidential(residentialComplex, location)).then(({
          ...
          dispatch(
            create({ location, ...data, residentialComplexId: id }, category),
          ),
        );
      }
      return dispatch(
        ...
        ).then(() => dispatch(create({ location, ...data }, category)));
    }
