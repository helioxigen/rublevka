...
export default reduxForm(formSettings)(submitValidator()(
  class extends Component {
    createAndLink(values) {
      const { actions, propertyId, propertyCategory } = this.props;
      const { kindId, ...otherValues } = values;

      return actions.createContact(otherValues)
        .then(contactId => actions.linkContact(propertyId, { kindId, linkedContactId: contactId }, propertyCategory))
            ...
            <Row className={sUtils.pushedBottom1}>
              <Col sm="20">
                <FormField field={fields.kindId} label="Роль" float>
                  <AsyncSelect asyncOptions={fetchDictionary(`property_contact_link_type`)} {...fields.kindId} valueKey="id" labelKey="title" />
                </FormField>
              </Col>
            </Row>
          </Container>
