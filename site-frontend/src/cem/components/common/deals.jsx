...
class Requests extends Component {
  componentWillMount() {
    const { actions, type = 'property', params: { id } } = this.props;
    const loadActions = {
  ...
  renderAgentFee({ expectedAgentFee, expectedAgentFixedPrice }) {
    ...
    } else if (expectedAgentFixedPrice && !!Object.keys(expectedAgentFixedPrice).length) {
      return <span><FormattedCurrency symbol={expectedAgentFixedPrice.currency} value={expectedAgentFixedPrice.price} /></span>;
    }
    return <span />;
  }

  render() {
    const { state, type = 'property', params: { id } } = this.props;
    const stateKeys = {
          ...
          <Table.Container width="100%">
            ...
            {items.map(item =>
                  (<Table.Row key={item.id}>
                    ...
                    <Table.Cell>{dict.offerKinds[item.details.offerKind]}</Table.Cell>
                    <Table.Cell>{dict.states[item.state].title}</Table.Cell>
                    <Table.Cell>{this.renderAgentFee(item.details)}</Table.Cell>
                    <Table.Cell><FormattedCurrency symbol={item.details.currency} value={item.details.budget} /></Table.Cell>
                    <Table.Cell>
                      <Button to={`/deals/${item.id}`} className={sButton.btnTableAction} size="xs">
