...
const { Button, Icon, Table, Tooltip } = UI;
...
const getOfferKind = ({ saleOffer = {}, rentOffer = {} }) => {
  ...
  const offers = [saleOffer, rentOffer].map(
    (offer, index) => offer.price && offerKinds[index],
  );
  return offers.filter(item => item).join(` / `);
};

const getPrice = ({ saleOffer = {}, rentOffer = {} }) => {
  const salePrice = saleOffer.price && (
    <FormattedCurrency symbol={saleOffer.currency} value={saleOffer.price} />
  );
  const rentPrice = rentOffer.price && (
      ...
      <Table.Row>
        ...
        <Table.Cell>
          <span
            className={
              s[
                dict.states[itemData.state] && dict.states[itemData.state].style
              ]
            }
          >
            {dict.states[itemData.state].title}
          </span>
        </Table.Cell>
        ...
        <Table.Cell>
          ...
          >
            <Button
              ...
              to={`/properties/${itemData.category}/${itemData.id}`}
              size="xs"
            >
