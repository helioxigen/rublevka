...
const ResponsibleUser = ({ data }) => (
  <section>
    ...
    <Media
      left={<ResponsibleUserPhoto id={data.photo && data.photo.id} />}
        ...
        <Col sm="16" md="18">
          <div className={s.taskWrapper}>
            <Row className={s.taskHeader}>
              <Col xs="20">
                <Heading size="lg" style={{ display: `inline-block` }}>
                  {dict.kinds[data.kind].title}
                </Heading>
                <Heading
                  size="md"
                  style={{ display: `inline-block`, marginLeft: `1.5rem` }}
                >
                  <span className={s[dict.states[data.state].style]}>
            ...
            <div className={s.positionTopRight}>
              {data.state === `to_do` && !stateDetails.toApprove && (
                <Button
                  size="xs"
                  kind="accent"
              ...
              <Button to={`/tasks/${data.id}`} size="xs">
                Подробнее
              </Button>
            </div>
              ...
              <Row>
                <Col sm="10">
                  <dl className={s.taskList}>
                    <dt>
                      <Heading size="md" className={s.taskTitle}>
                        Цель
                      </Heading>
                    </dt>
            ...
            {(data.kind === `negotiation` || data.kind === `preview`) &&
              !propertyCardHidden &&
              !!details.propertyId && (
                <Row>
