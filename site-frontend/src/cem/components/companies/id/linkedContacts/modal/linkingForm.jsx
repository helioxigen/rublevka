...
export default reduxForm(formSettings)(submitValidator()(
  class extends Component {
    ...
    link(values) {
      const { companyId, actions, onNextStep, resetPhoneNumber, closeModal } = this.props;

      return new Promise((resolve) => {
        API.get(`/v1/contacts`, { filter: { 'details.phoneNumber': values.details.phoneNumber } }).then(
          ({ body: { items } }) => {
            const contactData = items[0];
            if (contactData && contactData.companyDetails && contactData.companyDetails.companyId !== companyId) {
              resolve(actions.updateLinkedContact(companyId, contactData));
              resetPhoneNumber();
            } else if (contactData && contactData.companyDetails && contactData.companyDetails.companyId === companyId) {
              actions.pop(`warning`, `Контакт с этим номером уже привязан к данной компании`);
              resetPhoneNumber();
              closeModal();
            } else if (!contactData) {
