...
const formSettings = {
  form: `companyPhones`,
  fields: [`name`, `phone`],
  validate: ({ name, phone }) => ({ name: !name && `Обязательно`, phone: !phone && `Обязательно` }),
};

const Form = reduxForm(formSettings)(
  class extends Component {
    createOrUpdate() {
      const { formKey, values, addPhone, modifyPhone, destroyForm } = this.props;
    ...
    render() {
      const { formKey, fields, handleSubmit, removePhone, isStatic } = this.props;

      return (
        <Table.Row>
          <Table.Cell>
            <FormField className={sUtils.resetIndentation} field={fields.name} sideHelper>
              <Input className={s.tableInput} type="text" placeholder="Название" />
            </FormField>
          </Table.Cell>
          <Table.Cell>
            <FormField className={sUtils.resetIndentation} field={fields.phone} sideHelper>
              <Input className={s.tableInput} type="text" placeholder="Телефон" />
            </FormField>
          </Table.Cell>
          <Table.Cell>
            {formKey === `create` &&
            ...
            {formKey !== `create` &&
              <Tooltip className={sUtils.pushedRight1} title="Редактировать" position="top">
                <Button className={sButton.btnTableAction} onClick={handleSubmit(::this.createOrUpdate)} disabled={isStatic}>
                  <Icon className={s.btnIcon} icon="checkmark" />
                </Button>
              </Tooltip>
            }
            {formKey !== `create` &&
              <Tooltip title="Удалить" position="top">
                <Button className={sButton.btnTableAction} type="button" onClick={() => removePhone(formKey)} disabled={isStatic}>
        ...
        <Row className={sUtils.pushedBottom6}>
          <Col xs="20">
            <Table.Container width="100%">
              ...
              <Form formKey="create" addPhone={::this.addPhone} isStatic={isStatic} />
              {field.value && Object.keys(field.value).map(
                (key, index) => <Form formKey={key} initialValues={{ name: key, phone: field.value[key] }} key={index} modifyPhone={::this.modifyPhone} removePhone={::this.removePhone} isStatic={isStatic} />
              )}
            </Table.Container>
          </Col>
        </Row>
