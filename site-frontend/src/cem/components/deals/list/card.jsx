...
class Card extends Component {
  shouldComponentUpdate(nextProps) {
    ...
    return !isEqual(data, nextProps.data) || !isEqual(linkedContact, nextProps.linkedContact);
  }
  ...
  render() {
    ...
    return (
      <Link to={`/deals/${data.id}`} className={cn(s.card, data.stateDetails && data.stateDetails.toApprove && s[states[data.stateDetails.toApprove].style])}>
        <Container fluid>
          <Row>
            <Col xs="20">
              <h4 className={s.title}>
                ...
                <div className={sUtils.labelRight}>
                  ...
                  {isActive && data.tasks && !data.tasks.scheduled && (
                    <div className={sUtils.textRight}>
                      <Label kind="danger" className={sUtils.textUppercase}>нет задач</Label>
                    </div>
                  )}
                </div>
              </h4>
              ...
              <ParamList label="Клиент" small>{linkedContact && linkedContact.data && `${linkedContact.data.details.firstName || ''} ${linkedContact.data.details.lastName || ''}`}</ParamList>
            </Col>
          </Row>
          <Row>
            <Col sm="10">
            ...
            <Col sm="10">
              <ParamList label="Дата завершения" small>
                <FormattedDate mask="dd.mm.yyyy" value={data.details && data.details.expectedFinishDateAt} />
              </ParamList>
            </Col>
          </Row>
          <Row>
