...
class Header extends Component {
  createOrUpdate() {
    const { formKey, values: { aliases = [], ...values }, actions, kind } = this.props;
    const data = { aliases, ...values };

    if (formKey === `create`) {
      return actions.createPlace(kind, data).then(({ id }) => {
        actions.pushPath(`/places/${kind}/${id}`);
        actions.pop(`success`, `${dicts.kinds[kind].title[0]} (ID: ${id})`, `Успешно создано`);
      });
    }
    ...
    if (formKey !== `create`) {
      return actions.updatePlace(kind, formKey, data).then(() => {
        ...
        actions.pop(`success`, `${dicts.kinds[kind].title[0]} (ID: ${formKey})`, `Успешно обновлено`);
      });
    }
  }

  render() {
    const { formKey, fields, pristine, error, submitting, handleSubmit, kind, isUpdateAllowed } = this.props;

    return (
      <Form.Container className={s.header} onSubmit={handleSubmit(::this.createOrUpdate)}>
          ...
          <Row>
            <Col xs="20" className={sUtils.positionRelative}>
              <Heading size="lg">
                ...
                />
                {formKey === `create` && `Добавить ${dicts.kinds[kind].title[1].toLowerCase()}`}
                {formKey !== `create` && `${dicts.kinds[kind].title[0]} (ID: ${formKey})`}
              </Heading>
            </Col>
          </Row>
          <div className={s.media}>
            <Row>
              <Col xs="20">
                <Row>
                  <Col sm="7">
                    <FormField label={dicts.kinds[kind].title[0]} field={fields.name} float static={!isUpdateAllowed}>
