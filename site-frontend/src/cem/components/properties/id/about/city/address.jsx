...
import { findAddress, fetchResource, fetchAddress } from 'cem/helpers/autocomplete';
...
export default class extends Component {
  complete(option, value) {
    fetchAddress(1, [{ kladr_id: value.data.kladr_id }])(`${value.label}`, null, null).then(options => {
      const { actions, destroyForm, initializeForm, fields, values, data: propertyData } = this.props;
      ...
      const locationWithoutIds = {
        postalCode: data.postal_code, countryName: data.country,
        regionName: data.region, localityName: data.city,
        subLocalityName: data.city_district, street: data.street_with_type,
        house: house,
      ...
      destroyForm();
      initializeForm({ ...values, residentialComplex: {}, location: locationWithoutIds });
  ...
  render() {
    const { values, values: { location = {} }, data, fields, isUpdateAllowed } = this.props;
    const isAddressDisabled = values.saleOffer && values.saleOffer.isResale === `false`;

    const markerPosition = location.latitude && location.longitude ? { lat: location.latitude, lng: location.longitude } : {};
    ...
    return (
      <section className={this.props.className}>
        ...
        <Row>
          <Col lg="20">
            {isUpdateAllowed && !isAddressDisabled &&
              <Group kind={fields.location.countryId.touched && !!fields.location.countryId.error && `error`}>
                <Label>Поиск ЖК или адреса</Label>
                <AsyncSelect block asyncOptions={fetchResource(`/v1/complex_buildings`, `complexName`, (complexData) => `${complexData.complexName || `ЖК без названия`}, корпус ${complexData.location.building || `не задан`}`, {}, {}, findAddress)} onChange={::this.checkAddress} />
                {fields.location.countryId.touched && fields.location.countryId.error && <Helper>{fields.location.countryId.error}</Helper>}
              </Group>
            }
          </Col>
        </Row>
        <Row className={cn(sUtils.pushedBottom1, sUtils.pushedTop1)}>
          ...
          <Col xs="10" lg="5">
            <Group>
              ...
              <Static className={sUtils.fontSizeMd}>
                {location.subLocalityName || `—`}, {location.street || `—`}, {location.house || `—`}
              </Static>
            </Group>
          </Col>
          <Col xs="10" lg="2">
            <FormField field={fields.location.entrance} label="Подъезд" static={!isUpdateAllowed}>
              <Input className={sUtils.fontSizeMd} block type="text" />
            </FormField>
          </Col>
          <Col xs="10" lg="2">
            <FormField field={fields.location.flatNumber} label="Квартира" static={!isUpdateAllowed}>
              <Input className={sUtils.fontSizeMd} block type="text" />
            </FormField>
          </Col>
          <Col xs="10" lg="7" lgOffset="1">
            <FormField label="Метро" field={fields.location.subwayIds} asyncValue={fetchSubways} labelKey="name" static={values.complexBuildingId || !isUpdateAllowed}>
        ...
        {values.complexBuildingId &&
          <Row>
            <Col xs="20">
              <ComplexBuildingCard id={values.complexBuildingId} alternativeName={data.complexBuildingName} />
            </Col>
          </Row>
        }
