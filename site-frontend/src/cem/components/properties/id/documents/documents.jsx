  ...
  submitValidator()(
    class extends Component {
      ...
      createOrUpdate() {
        ...
        if (formKey === `add`)
          return actions.linkDocument(propertyId, values, category);
        if (formKey !== `add`)
          return actions.updateDocument(propertyId, formKey, values, category);
      }
      ...
      render() {
        ...
        return (
          <Table.Row>
            <Table.Cell>
              ...
              >
                <AsyncSelect
                  ...
                  {...fields.kindId}
                  asyncOptions={fetchDictionary(`property_document_type`)}
                  labelKey="title"
                  valueKey="id"
                  onBlur={() => {}}
                />
            ...
            <Table.Cell>
              {formKey !== `add` && <Grid.Row>{values.uploaderTitle}</Grid.Row>}
            </Table.Cell>
            <Table.Cell>
              <Grid.Row>
                <Col xs="16">
              ...
              <Table.Cell>
                <Group className={sUtils.resetIndentation}>
                  ...
                  <Button
                    className={sButton.btnTableAction}
                    size="xs"
                    to={`${API}/v1/properties/${category}/${propertyId}/documents/${formKey}/download?token=${
                      state.auth.token
                    }`}
                    target="_blank"
                  >
