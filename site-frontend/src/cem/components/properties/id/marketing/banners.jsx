...
class Banner extends Component {
  update() {
    ...
    actions.updateBanner(propertyId, category, formKey, values).then(() => {
  ...
  render() {
    ...
    return (
      <Row>
        ...
        <Cell>
          <StaticDictionary
            fetch={fetchDictionary('property_banner')}
            value={data.kindId}
            labelKey="title"
            valueKey="id"
          />
        ...
        {(bannerState === 'ordered' ||
          ...
          <Cell>
            ...
            {bannerState === 'ordered' && isUserResponsible && (
              <ModalApprove
                {...this.props}
                formKey={data.id}
                data={data}
                initialValues={{ ...data, state: 'active' }}
              />
            )}
            {bannerState === 'active' && isUserResponsible && (
              <ModalReject
                {...this.props}
                formKey={data.id}
                data={data}
                initialValues={{ ...data, state: 'removed' }}
              />
            )}
          </Cell>
        )}
  ...
  render() {
    ...
    return (
      <Grid.Row className={sUtils.pushedBottom3}>
        <Col xs="20">
          <Heading size="sm" className={sUtils.resetIndentation}>
            {bannerStatesPlural[bannerState]}
          </Heading>
        </Col>
        <Col xs="20">
          {items && !!items.length && (
            <div className={sUtils.scrollX}>
              <Table.Container
                width="100%"
                className={cn(sUtils.pushedBottom1, sUtils.width120)}
              >
