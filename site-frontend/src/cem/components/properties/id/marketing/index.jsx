...
class Marketing extends Component {
  render() {
    ...
    const isBannersPresent = !!Object.keys(banners).filter(
      key => banners[key].items && !!banners[key].items.length,
    ).length;
    ...
    return (
      <Grid.Row>
        <section className={s.section}>
              ...
              <Heading size="md" className={sUtils.pushedBottom1_5}>
                ...
                {isUpdateAllowed && (
                  <ModalMarketing
                    ...
                    actions={actions}
                    responsibleUserId={state.auth.id}
                  />
                )}
              </Heading>
            </Col>
          ...
          {!isBannersPresent && !isBannersFetching && data.id && (
            <Heading notFound className={sUtils.pushedTop1}>
              Нет внешней рекламы
            </Heading>
          )}
          {data.id && (
