...
const {
  Form, Button, Daypicker, Icon,
  Modal, Tooltip, Heading,
  Form: { Input },
...
const ModalInner = reduxForm(formSettings)(submitValidator()(
  class extends Component {
    update() {
      const { formKey, propertyId, category, data, values: { image, ...values }, closeModal, actions } = this.props;
      const bannerData = { ...data, ...values };

      actions.updateBanner(propertyId, category, formKey, bannerData)
        .then(() => {
          actions.uploadBanner(propertyId, category, formKey, image.url)
            .then(() => {
      ...
      return (
        <Form.Container onSubmit={handleSubmit(::this.update)}>
          <div className={s.container}>
            ...
            <Row className={sUtils.pushedTop3}>
              <Col xs="20">
                <FormField field={fields.dateOfCompletion} label="Дата установки">
                  <Daypicker className={cn(sUtils.pushedTop1_2, sUtils.fullWidth, sDaypicker.daypicker)} restrict="past"
                    control={<Input block {...fields.dateOfCompletion} type="text" required />}
            ...
            <Row>
              <Col xs="20">
                <FormField className={sUtils.resetIndentation} field={fields.image.url} label="Фотография">
                  <Uploadcare keepValue>
                    <Button type="button" className={cn(sMain.btnBigPlus, sUtils.pushedTop1_2)} block>
          ...
          <Button className={sButton.btnWide} kind="success" size="lg" block>Сохранить</Button>
        </Form.Container>
      );
    }
  }
    ...
    return (
      <div className={s.modalWrapper}>
        <Tooltip title="Установить рекламу" position="top">
          <Button type="button" className={cn(sButton.btnTimes, sUtils.resetIndentation)} block size="xs" onClick={() => this.toggleModal(true)}>
