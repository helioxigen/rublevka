...
import {
  PROPERTY_BANNERS_OPENED,
  PROPERTY_BANNERS_CREATED,
    ...
    class extends Component {
      ...
      create() {
        const {
          propertyId,
          category,
          values: { image, state, ...values },
          actions,
          responsibleUserId,
        } = this.props;
        const messages = {
          ordered: `Реклама заказана`,
          denied: `В рекламе отказано`,
          active: `Реклама размещена`,
        };

        actions
          .createBanner(propertyId, category, { responsibleUserId, ...values })
          .then(({ bannerId }) => {
            ...
            if (state === `active`) {
              ...
              actions
                .uploadBanner(propertyId, category, bannerId, image.url)
                .then(() => {
            ...
            <div className={s.container}>
                ...
                <Col xs="20">
                  <FormField field={fields.kindId} label="Тип" float>
                    <AsyncSelect
                      labelKey="title"
                      valueKey="id"
                      asyncOptions={fetchDictionary(`property_banner`)}
                    />
                  </FormField>
                </Col>
              </Row>
              ...
              {fields.state.value === `ordered` && (
                <Row>
                  <Col xs="20">
                    <FormField
                      field={fields.expectedDateOfCompleteion}
                      label="Ожидаемая дата установки"
                    >
                      <Daypicker
                        ...
                        control={
                          <Input
                            block
                            {...fields.expectedDateOfCompleteion}
                            type="text"
                          />
                        }
                        button={
              ...
              {fields.state.value === `denied` && (
                <FormField field={fields.reason} label="Причина">
                  <Textarea
                    className={cn(s.textarea, sUtils.pushedTop1_2)}
                    block
                    rows="3"
                  />
                </FormField>
  ...
  render() {
    return (
      <div className={s.modalWrapper}>
        <Button
          type="button"
          className={cn(sButton.btnRoundPlus, sUtils.pushedLeft1)}
