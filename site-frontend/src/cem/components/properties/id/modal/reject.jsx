...
const {
  Form, Button, Modal, Icon, Tooltip,
  Heading,
  Form: { Textarea },
...
const ModalInner = reduxForm(formSettings)(submitValidator()(
  class extends Component {
    update() {
      const { formKey, propertyId, category, data, values, closeModal, actions } = this.props;
    ...
    render() {
      ...
      return (
        <Form.Container onSubmit={handleSubmit(::this.update)}>
          ...
          <Button className={sButton.btnWide} kind="danger" size="lg" block>Снять</Button>
        </Form.Container>
      );
    }
  }
    ...
    return (
      <div className={cn(s.modalWrapper, sUtils.pushedLeft1)}>
        <Tooltip title="Снять рекламу">
          <Button type="button" className={sButton.btnTimes} block size="xs" onClick={() => this.toggleModal(true)}>
            <Icon className={s.btnIcon} icon="delete" />
          </Button>
        </Tooltip>
        <Modal size="md" closeOnEsc closeOnOutsideClick onClose={() => this.toggleModal(false)} isOpened={this.state.isOpen} closePortal={() => this.toggleModal(false)}>
