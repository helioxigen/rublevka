...
import { propertyCategories, states } from 'cem/constants/requests/search/dictionaries';

export default ({ data, creatorUserData = {}, responsibleUserData = {} }) => (
  <Link to={`/requests/properties/search/${data.id}`} className={cn(s.card, s[states[data.state].style])}>
    <div className={s.cardWrapper}>
      <Container fluid className={s.flex}>
        <Row>
          <Col sm="2">
            <ParamList label="Тип" big>{propertyCategories[data.propertyCategory]}</ParamList>
          </Col>
          <Col sm="4">
