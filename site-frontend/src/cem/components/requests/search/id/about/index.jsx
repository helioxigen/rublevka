...
class About extends Component {
  handlePropertiesAddition(properties) {
    ...
    handleSubmit(() => {
      return actions.updateSearchRequest(formKey, data)
        .then(() => {
  ...
  render() {
    const { formKey, fields, data = {}, isUpdateAllowed, isCommentingAllowed } = this.props;

    return (
      <Container fluid className={s.section}>
        <Request {...this.props} />
        {formKey !== `create` && data.state !== `new` && data.state !== `assigned` &&
          <Row>
            <Col xs="20">
              <Heading size="md">
                Подходящие объекты
                {data.state === `in_progress` &&
                  <ModalAddProperty onAdd={::this.handlePropertiesAddition} propertyCategory={data.propertyCategory} field={fields.propertyIds}>
        ...
        {formKey !== `create` && data.state !== `new` && data.state !== `assigned` && <Properties field={fields.propertyIds} toggle={fields.toggle} propertyCategory={data.propertyCategory} isStatic={data.state !== `in_progress` || !isUpdateAllowed} title="Подходящие объекты" />}
        {formKey !== `create` && data.state !== `new` && data.state !== `assigned` && <LeadsTable searchRequestId={data.id} category={data.propertyCategory} isStatic={data.state !== `in_progress` || !isUpdateAllowed} />}
        ...
        {data.state === `finished` && data.id && <Answers kind="selection" requestId={data.id} />}
        <Row>
          <Col sm="10">
            {data.createdByUser && <User id={data.createdByUser.id} title="Заказчик" />}
          </Col>
          <Col className={sUtils.pushedTopXs2} sm="10">
            {data.responsibleUser && <User id={data.responsibleUser.id} title="Ответственный" />}
          </Col>
        </Row>
      </Container>
    );
