...
class About extends Component {
  render() {
    const { fields, data = {}, formKey, isUpdateAllowed, isSensitiveDataVisible } = this.props;
    const state = data.state;
    const stateDetails = data.stateDetails || {};

    const isStatic = (formKey !== 'create' && !isUpdateAllowed) || stateDetails.toApprove || (['spam', 'processed', 'rejected'].indexOf(state) > -1);
          ...
          <Col xs="20">
            <Form.Container className={sModal.container}>
              <div className={sUtils.height60}>
                {(formKey === 'create' || isSensitiveDataVisible) && <ContactDetails fields={fields.contactDetails} isStatic={isStatic} isPhoneStatic={isPhoneStatic} data={data} />}
