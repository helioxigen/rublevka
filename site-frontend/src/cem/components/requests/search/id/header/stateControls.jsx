...
const SubmitButton = ({ children, className, kind, disabled, ...props }) => (
  <Button
    {...props}
    className={cn(s.btnWide, className)}
    kind={kind}
    type="submit"
    size="lg"
    block
  ...
  <Row xs="center">
    <Col xs="20">
      {state === 'new' && isCurrentUserSupervisor && (
        <ModalChangeState
          action={val => changeState('assign', val)}
          submitBtn={
      ...
      {state === 'new' && isCurrentUserSupervisor && (
        <ModalCanceled
          ...
          kind="reject"
          submitBtn={
            <SubmitButton className={sButton.btnWide} kind="danger">
              Отклонить заявку
            </SubmitButton>
          }
        >
          <Button type="button" kind="danger" size="xs">
            отклонить заявку
          </Button>
        </ModalCanceled>
      )}
      {state === 'assigned' && isCurrentUserResponsible && (
        <Button
          ...
          type="button"
          size="xs"
          onClick={() => changeState('in_progress', { reason: null })}
        >
          взять в работу
        </Button>
      )}
      ...
      {state === 'approved' && isCurrentUserCreator && (
        <ModalFinished
          ...
          action={val => changeState('finished', val)}
          submitBtn={
            <SubmitButton className={sButton.btnWide} kind="success">
              Подтвердить
            </SubmitButton>
      ...
      {((state === 'done' && isCurrentUserSupervisor) ||
        ...
        <ModalReturn
          ...
          kind="return"
          submitBtn={
            <SubmitButton kind="warning" className={sButton.btnWide}>
