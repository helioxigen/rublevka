...
class Lane extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const nextPagination = nextProps.state.pagination[`searchRequests.${laneKey}`] || {};
    ...
    if (isPaginationUpdated || isFilterUpdated) {
      actions.loadSearchRequests(laneKey, { pagination: { offset: nextPagination.offset, limit: nextPagination.limit }, filter: nextFilter });
    }
  }
  ...
  render() {
    ...
    return (
      <section>
        {items.map(item =>
          <Card key={item.id} data={item} creatorUserData={state.users[item.createdByUser.id] && state.users[item.createdByUser.id].data} responsibleUserData={item.responsibleUser && state.users[item.responsibleUser.id] && state.users[item.responsibleUser.id].data} />,
        )}
        <Container fluid>
          <Row xs="center">
            <Col className={cn(s.pushedTop2, s.pushedBottom2)}>
              {!isFetching && pagination.total > pagination.limit && (
                <Button size="xs" kind="accent" onClick={() => actions.updatePagination(`searchRequests.${laneKey}`, { offset: pagination.offset + pagination.limit })}>Загрузить следующие</Button>
              )}
              {isFetching && <Loading />}
            </Col>
          </Row>
