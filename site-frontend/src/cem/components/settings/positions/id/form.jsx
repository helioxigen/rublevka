...
import {
  ...
  scopeTitle,
  permissionScopes,
} from 'cem/constants/settings/positions/dictionaries';
    ...
    return (
      <Table.Row>
            ...
            <Input
              ...
              onChange={() =>
                isSelected
                  ...
                  : this.props.onUpdate(id, this.props.availableScopes[0])
              }
            />
        ...
        <Table.Cell>
          {isScopeSelectionPossible && (
            <FormField className={sUtils.resetIndentation}>
              <Select
                ...
                onChange={value => this.props.onUpdate(id, value)}
                value={fieldsScope}
                disabled={disabled}
              />
            </FormField>
          ...
          {!isScopeSelectionPossible && (
            <i>{scopeTitle[this.props.availableScopes[0]]}</i>
          )}
        </Table.Cell>
      </Table.Row>
    );
  ...
  render() {
    ...
    return (
      <Table.Container width="100%">
        <Table.Row>
          <Table.Heading width="85%">
            {permissionCategoryTitle[categoryId]}
          </Table.Heading>
          <Table.Heading width="15%">Область полномочий</Table.Heading>
        </Table.Row>
        {availablePermissions.map(permission => {
          const permissionId = permission.useTypeAsPermissionId
            ? permission.type
            : categoryId + `_` + permission.type;
          return (
            <PermissionField
              id={permissionId}
              key={permissionId}
              ...
              label={
                permission.messageType
                  ? permissionType[permission.messageType]
              ...
              fieldsScope={
                ...
                fields.permissions.value[permissionId]
              }
...
class PositionForm extends Component {
  update() {
    return this.props.actions.updatePosition(
  ...
  render() {
    ...
    const selectedPermissionsMap = fields.isAdmin.value
      ? Object.keys(permissionScopes).reduce(
          (resultPermissions, permissionId) => ({
            ...resultPermissions,
        ...
        {Object.keys(permissionsPerCategory).map(categoryId => (
          <Grid.Row className={sUtils.pushedTop3} key={categoryId}>
            <Col xs="20" className={sUtils.pushedBottom3}>
              <PermissionsGrid
                disabled={fields.isAdmin.value || !isUpdateAllowed}
