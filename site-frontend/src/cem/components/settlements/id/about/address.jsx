...
class Address extends Component {
  ...
  render() {
    const { fields, values: { location = {} }, hasRight } = this.props;
          ...
          <Col sm="10">
            <Row>
              <Col lg="16" className={sUtils.pushedBottom3}>
                <Row>
                  <Col xs="20">
                    <Group kind={fields.location.localityId.touched && !!fields.location.localityId.error && `error`}>
                      <Label>Населенный пункт</Label>
                      <AsyncSelect asyncOptions={fetchResource(`/v1/places/localities`, `name`)} {...fields.location.localityId} onChange={(...params) => ::this.changeSubLocality(...params)} disabled={!hasRight(`settlement_update`)} />
                      {fields.location.localityId.touched && fields.location.localityId.error && <Helper>{fields.location.localityId.error}</Helper>}
                    </Group>
                  </Col>
                  <Col xs="20">
                    <Group kind={fields.location.linkedLocalityIds.touched && !!fields.location.linkedLocalityIds.error && `error`}>
                      <Label>Доп. населенные пункты</Label>
                      <AsyncSelect asyncOptions={fetchResource(`/v1/places/localities`, `name`)} multi {...fields.location.linkedLocalityIds} disabled={!hasRight(`settlement_update`)} />
                      {fields.location.linkedLocalityIds.touched && fields.location.linkedLocalityIds.error && <Helper>{fields.location.linkedLocalityIds.error}</Helper>}
                    </Group>
                  </Col>
                </Row>
                <Row>
                  <Col sm="10">
                    <FormField float label="Широта" field={fields.location.latitude} static={!hasRight(`settlement_update`)}>
                      <Input block type="text" />
                    </FormField>
                  </Col>
                  <Col sm="10">
                    <FormField float label="Долгота" field={fields.location.longitude} static={!hasRight(`settlement_update`)}>
                      <Input block type="text" />
                    </FormField>
                  </Col>
                </Row>
                <Row>
                  <Col sm="10">
                    <FormField float label="От МКАД, км" field={fields.location.mkadDistance} static={!hasRight(`settlement_update`)}>
                      <Input block type="text" />
                    </FormField>
                  </Col>
                </Row>
