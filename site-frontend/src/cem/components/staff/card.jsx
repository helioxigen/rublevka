...
const Description = ({ user }) => (
  <div>
    ...
    {user.details && user.details.roleName && (
      <p className={s.position}>{user.details.roleName}</p>
    )}
  </div>
);
    ...
    <div className={s.cardWrapper}>
      <Container fluid className={s.flex}>
        <Row>
          <Col sm="16">
            <Media
              left={<Image photo={user.photo} />}
              body={<Description user={user} />}
            />
          </Col>
        </Row>
        <Row>
          <Col className={sUtils.pushedTopXs2} sm="2" md="1">
            <ParamList label="ID">{user.id}</ParamList>
          </Col>
          {user.details && user.details.departmentName && (
          ...
          {user.details && user.details.divisionName && (
            <Col className={sUtils.pushedTopXs2} sm="8" md="6">
              <ParamList label="Отдел">{user.details.divisionName}</ParamList>
            </Col>
          )}
          {/* <Col sm="4">
