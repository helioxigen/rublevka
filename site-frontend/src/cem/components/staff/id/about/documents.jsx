        ...
        <Table.Row>
          <Table.Cell>
            <Group className={sUtils.resetIndentation} kind={fields.kindId.touched && !!fields.kindId.error && `error`}>
              <AsyncSelect className={sUtils.resetBorder} {...fields.kindId} asyncOptions={fetchDictionary(`staff_document_type`)} labelKey="title" valueKey="id" onBlur={() => {}} />
          ...
          {formKey !== `add` && (
            <Table.Cell>
              <Group className={sUtils.resetIndentation}>
                ...
                <Button className={sButton.btnTableAction} size="xs" to={`${API}/v1/users/staff/${this.props.resourceId}/documents/${formKey}/download?token=${this.props.state.auth.token}`} target="_blank">
                  <Icon className={s.btnIcon} icon="download" />
                </Button>
                <Button className={sButton.btnTableAction} size="xs" onClick={handleSubmit(::this.remove)}>
      ...
      <section>
        ...
        <div className={sUtils.scrollX}>
          <Table.Container width="100%" className={sUtils.width120}>
            ...
            {items.map(item => <DocumentForm {...this.props} key={item.id} formKey={item.id.toString()} initialValues={item} />)}
          </Table.Container>
        </div>
      </section>
    );
