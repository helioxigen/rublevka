...
const { Grid, Table, Loading, Heading } = UI;
...
export default class extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      id !== nextProps.id &&
      state.users[nextProps.id].data.details.isManager
    ) {
  ...
  render() {
    ...
    const { ids = [], isFetching } =
      state.users.subordinateUsersByUserId[userId] || {};
    ...
    return (
      <Grid.Row>
        <section className={s.section}>
          {state.users[userId].data.details.isManager &&
            !isFetching &&
            ids &&
            !!ids.length && (
              <div className={sUtils.scrollX}>
                <Table.Container width="100%" className={sUtils.width110}>
                  ...
                  {ids.map(subordinateUserId => {
                    ...
                    return (
                      <Table.Row>
                        <Table.Cell>{`${subordinateUser.firstName} ${
                          subordinateUser.lastName
                        }`}</Table.Cell>
                        <Table.Cell>
                          {subordinateUser.details.roleName ? (
                            subordinateUser.details.roleName
                          ) : (
                            <i>Не определена</i>
                          )}
                        ...
                        <Table.Cell>
                          {subordinateUser.details.divisionName ? (
                            subordinateUser.details.divisionName
                          ) : (
                            <i>Не определён</i>
                          )}
                        ...
                        <Table.Cell
                          className={s[states[subordinateUser.state].style]}
                        >
                          {states[subordinateUser.state].title}
                        </Table.Cell>
                      </Table.Row>
                    );
                  })}
          ...
          {(!state.users[userId].data.details.isManager ||
            (!isFetching && ids && !ids.length)) && (
            <Heading notFound>Подчиненных сотрудников нет</Heading>
          )}
        </section>
      </Grid.Row>
    );
