...
class Form extends Component {
  create() {
    const { taskId, actions, values: { file } } = this.props;
  ...
  render() {
    ...
    return (
      <Row>
        {data && <Cell><FormattedDate mask="dd.mm.yy HH:MM" value={data.createdAt} /></Cell>}
        {!data &&
          <Cell>
        ...
        <Cell>{data && <FormattedDate mask="dd.mm.yy HH:MM" value={data.archivedAt} />}</Cell>
        <Cell>
          <Group className={sUtils.resetIndentation}>
            {!data &&
              <Button className={sButton.btnTableAction} size="xs" type="button" onClick={handleSubmit(::this.create)}>
                <Icon className={s.btnIcon} icon="checkmark" />
              </Button>
            }
            {data && !data.archivedAt &&
              <Button className={sButton.btnTableAction} size="xs" type="button" onClick={::this.archive}>
                <Icon className={s.btnIcon} icon="archive" />
              </Button>
            }
            {data &&
              <a className={sButton.btnTableAction} size="xs" type="button" href={`${API}/v1/tasks/${taskId}/document/download?token=${auth.token}`}>
