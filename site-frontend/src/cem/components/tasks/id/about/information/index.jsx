  ...
  render() {
    ...
    const isLinkedToProperty =
      formKey === 'create'
        ...
        : (data.freeDetails && data.freeDetails.linkKind === 'property') ||
          (data.contactDetails && data.contactDetails.linkKind === 'property');
    const isLinkedToDeal =
      formKey === 'create'
        ? !!dealId
        : (data.freeDetails && data.freeDetails.linkKind === 'deal') ||
          (data.contactDetails && data.contactDetails.linkKind === 'deal') ||
          !!data.previewDetails ||
          !!data.negotiationDetails;
    const isLinkedToLead =
      formKey === 'create'
        ? !!clientLeadId
        : (data.freeDetails && data.freeDetails.linkKind === 'client_lead') ||
          (data.contactDetails &&
            data.contactDetails.linkKind === 'client_lead');
    ...
    const isContactSelectionShown =
      formKey === 'create' &&
      ((!!detailsFields.properties && !!detailsFields.properties.value) ||
        (!!detailsFields.propertyId && !!detailsFields.propertyId.value));
    ...
    return (
      <Row className={sUtils.pushedBottom6}>
        {isLinkedToProperty && !isPreview && !isNegotiation && (
          <Col sm="10" md="6">
            <PropertyCard
              id={detailsFields.propertyId.value}
              resourcePath={`/v1/properties/${
                detailsFields.propertyCategory.value
              }`}
            />
          </Col>
        )}
            ...
            <Heading size="md">
              ...
              {detailsFields.contactId && detailsFields.contactId.value && (
                <Link
                  ...
                  to={`/contacts/${detailsFields.contactId.value}`}
                >
            ...
            {isContactSelectionShown && isLinkedToProperty && (
              <FormField field={detailsFields.contactId}>
                <Select
                  ...
                  valueKey="linkedContactId"
                  options={state.contactsByPropertyId[propertyId] || []}
                />
              </FormField>
            )}
            {isContactPreviewShown && (
              <Contact id={detailsFields.contactId.value} />
            )}
          </Col>
        )}
