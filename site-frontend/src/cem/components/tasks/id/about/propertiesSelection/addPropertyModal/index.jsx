...
class ModalAddProperty extends Component {
  ...
  add() {
    ...
    const properties = values.properties.filter(item => !field.find(fieldItem => fieldItem.propertyId === item.propertyId));
  ...
  render() {
    const { fields, values, children, currentLimit, totalLimit } = this.props;
        ...
        <Modal size="lg" closePortal={::this.close} isOpened={this.state.isOpened} onClose={::this.close} closeOnEsc closeOnOutsideClick>
          <div className={s.container}>
            <Row>
              ...
              <Col xs="4">
                <Heading size="md" className={sUtils.textRight}>{totalLimit - currentLimit + currentPropertiesCount}/{totalLimit}</Heading>
              </Col>
            </Row>
            <Row className={sUtils.pushedTop3}>
              <Col xs="20">
                <PropertyCardsList field={fields.properties} limit={currentLimit} />
              </Col>
            </Row>
          </div>
          <Button className={sButton.btnWide} kind="success" type="button" size="lg" block onClick={::this.add} disabled={!currentPropertiesCount}>
            Добавить <CountIndicator count={currentPropertiesCount} declensionForms={[`объект`, `объекта`, `объектов`]} />
          </Button>
        </Modal>
      </div>
    );
