...
class Properties extends Component {
  ...
  render() {
    const { field, isStatic, limit } = this.props;
    ...
    const isLimitReached = !!field.value ? limit - field.value.length <= 0 : false;
    ...
    return (
      <section className={sUtils.pushedBottom3}>
        ...
        {!!field.value && !!field.value.length && field.value.map(({ propertyId, propertyCategory }, index) =>
          <Row key={index} className={cn(sUtils.pushedBottom2)}>
            <Col xs="20">
              <Property isPreview id={propertyId} resourcePath={`/v1/properties/${propertyCategory}`} isStatic={isStatic} handleDelete={() => ::this.handlePropertyRemove(propertyId)} />
            </Col>
          </Row>
        )}
      </section>
