...
class PropertiesList extends Component {
  ...
  render() {
    const {
      ...
      formKey,
      detailsField,
      detailsField: { propertyId, propertyCategory },
      isUpdateAllowed,
    } = this.props;
        ...
        <Row>
          <Col xs="20">
            <Heading size="md">
              ...
              {(formKey === `create` ||
                ...
                !isLimitReached && (
                  <ModalAddProperty
                    ...
                    totalLimit={previewPropertiesLimit}
                  >
                    <Button
                      type="button"
              ...
              <Group
                kind={
                  detailsField.propertyId.touched &&
                  !!detailsField.propertyId.error &&
                  `error`
                }
              >
                {detailsField.propertyId.touched &&
                  detailsField.propertyId.error && (
                    <Helper>{detailsField.propertyId.error}</Helper>
                  )}
              </Group>
            </Heading>
          </Col>
          ...
          <Col xs="20">
            {!!propertyId.value && (
              <Property
                isPreview
                id={propertyId.value}
                resourcePath={`/v1/properties/${propertyCategory.value}`}
                handleDelete={() =>
                  ::this.handlePropertyRemove(propertyId.value)
                }
                isStatic={isStatic}
              />
            )}
  ...
  render() {
    ...
    return (
      <PropertiesList
        {...this.props}
        detailsField={
          isPreview ? fields.previewDetails : fields.negotiationDetails
        }
      />
    );
  }
...
export default reduxForm(taskFormSettings)(
  submitValidator()(PropertiesSelectionWrapper),
);
