...
class Header extends Component {
  ...
  handleApprove() {
    ...
    actions
      .changeStatus(formKey, `approve`)
      .then(() => actions.loadTask(formKey));
  }
  ...
  handleDisapprove() {
    ...
    actions
      .changeStatus(formKey, `disapprove`)
      .then(() => actions.loadTask(formKey));
  }
  ...
  changeResponsibleUser(id) {
    ...
    handleSubmit(
      () => actions.transfer(`tasks`, data.id, id),
  ...
  render() {
    ...
    const isDeadlineEditable =
      isUpdateAllowed &&
      formKey !== `create` &&
      Math.abs(moment(createdAt).diff(moment(), `minutes`)) <= 60;
    const showControls =
      formKey !== `create` &&
      fields.state.value === `to_do` &&
      isUpdateAllowed &&
      !stateDetails.toApprove;
    const showManagerControls =
      formKey !== `create` &&
      fields.state.value === `to_do` &&
      data.reportedByUserId === auth.id &&
      stateDetails.toApprove;
    ...
    return (
      <section>
            ...
            <Row className={s.formContainer}>
              <Col md="14" mdOffset="3">
                <Row>
                    ...
                    <Col sm="6" md="3" mdOffset="3">
                      ...
                      {!stateDetails.toApprove && (
                        <Group>
                          <Form.Label block>Статус</Form.Label>
                          <Static
                            className={cn(s.input, s[state && state.style])}
                          >
                            {state && state.title}
                          </Static>
                        </Group>
                  ...
                  {(formKey === `create` ||
                    (fields.state.value === `to_do` && isDeadlineEditable)) && (
                    <Col sm="6" smOffset="1">
                      <Group
                        inline
                        kind={
                          fields.deadline.date.touched &&
                          !!fields.deadline.date.error &&
                          `error`
                        }
                      >
                        ...
                        <Daypicker
                          ...
                          control={
                            <Input
                              className={sDaypicker.tasksInputDaypicker}
                              type="text"
                        ...
                        {fields.deadline.date.touched &&
                          fields.deadline.date.error && (
                            <Helper className={s.helperDaypicker}>
                              {fields.deadline.date.error}
                            </Helper>
                          )}
                      </Group>
                      ...
                      <Col sm="6" smOffset="1" md="4" mdOffset="2">
                        <Group>
                          ...
                          <Static className={s.input}>
                            <FormattedDate
                              value={fields.deadline.date.value || 0}
                              mask="dd.mm.yyyy"
                            />
                          </Static>
                        </Group>
                  ...
                  <Col sm="6" smOffset="1">
                    {(formKey === `create` ||
                      ...
                      <Group
                        kind={
                          fields.deadline.time.touched &&
                          fields.deadline.time.error &&
                          `error`
                        }
                      >
                        ...
                        {fields.deadline.time.touched &&
                          fields.deadline.time.error && (
                            <Helper>{fields.deadline.time.error}</Helper>
                          )}
                      </Group>
                    )}
                    {formKey !== `create` &&
                      (fields.state.value !== `to_do` ||
                        ...
                        <Group>
                          ...
                          <Static className={s.input}>
                            {fields.deadline.time.initialValue}
                          </Static>
                        </Group>
                      )}
                  </Col>
                ...
                {stateDetails.toApprove && data.result && (
                  <Row>
                    <Col sm="10" smOffset="5">
                      <Heading size="sm">
                        {`${dict.states[stateDetails.toApprove].title}`},
                        результат:
                      </Heading>
        ...
        <Modal
          ...
          isOpened={this.state.openedModalName === `done`}
          closePortal={::this.closeAllModals}
        >
          <ResultForm
            taskId={id}
            status="done"
            ...
            actions={actions}
            onSubmitResult={::this.closeAllModals}
          />
        </Modal>
        <Modal
          size="sm"
          ...
          closeOnOutsideClick
          onClose={::this.closeAllModals}
          isOpened={this.state.openedModalName === `cancel`}
          closePortal={::this.closeAllModals}
        >
          <ResultForm
            taskId={id}
