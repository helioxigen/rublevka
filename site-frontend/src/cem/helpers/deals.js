...
export const mapFilter = (filter = {}) => {
  ...
  return {
    ...
    'details.expectedFinishDateAt': makeDateRange(formatFilterDate(expectedFinishAtFrom), formatFilterDate(expectedFinishAtTo)),
  };
};
...
export const mapLaneFilterAndFilterNot = (filter = {}, filterNot = {}, kind) => {
  ...
  if (kind === 'approval') {
    ...
    const result = {
      ...
      filterNot: !!filter.state && filter.state !== 'approval' ? { ...filterNot, 'stateDetails.toApprove': 'successful,unsuccessful' } : filterNot,
    };
...
export const calculateToApproveAgencyFee = (stats, state, currency) => stats.toApprove[state] ? Object.keys(stats.toApprove[state]).reduce((result, key) => result + stats.toApprove[state][key][currency], 0) : 0;
