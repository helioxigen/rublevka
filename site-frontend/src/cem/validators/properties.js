...
const validate = (property) => {
  ...
  if (values.state && values.state !== 'draft') {
    ...
    if (values.rentOffer) {
      if (!values.rentOffer.price || values.rentOffer.price === '0') errors.rentOffer.price = 'Укажите валюту';
      ...
      if (!values.rentOffer.deposit && values.rentOffer.deposit !== 0) errors.rentOffer.deposit = 'Укажите размер депозита';
      if (!values.rentOffer.isAgentFixed) errors.rentOffer.isAgentFixed = 'Укажите формат агентской комиссии';
      if (values.rentOffer.isAgentFixed === 'false') {
        if (normalizeNumber(values.rentOffer.agentFee) > 100) errors.rentOffer.agentFee = 'Комиссия не может быть больше 100%';
      } else if (values.rentOffer.agentFixedPrice) {
        if (!values.rentOffer.agentFixedPrice.price) errors.rentOffer.agentFixedPrice.price = 'Укажите цену';
        if (!values.rentOffer.agentFixedPrice.currency) errors.rentOffer.agentFixedPrice.currency = 'Укажите валюту';
      } else {
    ...
    // See :19
    if (values.saleOffer) {
      if (!values.saleOffer.price || values.saleOffer.price === '0') errors.saleOffer.price = 'Укажите валюту';
    ...
    if (values.category === 'city') {
      if (values.specification) {
        if (!values.specification.totalArea) errors.specification.totalArea = 'Укажите общую площадь';
        if (!values.specification.livingArea) errors.specification.livingArea = 'Укажите жилую площадь';
    ...
    if (values.category === 'country' && values.kind !== 'land') {
      ...
      if (values.kind !== 'land') {
        if (values.specification) {
          if (!values.specification.wallMaterial) errors.specification.wallMaterial = 'Укажите конструкцию дома!';
          if (!values.specification.roofMaterial) errors.specification.roofMaterial = 'Укажите материал крыши!';
          if (!values.specification.bedrooms) errors.specification.bedrooms = 'Укажите общее количество спален!';
          if (!values.specification.area) errors.specification.area = 'Укажите площадь!';
          if (!values.specification.builtYear) errors.specification.builtYear = 'Укажите год строительства!';
          if (!values.specification.floors) errors.specification.floors = 'Укажите количество этажей!';
          if (!values.specification.renovate) errors.specification.renovate = 'Укажите состояние ремонта!';
          if (!values.specification.condition) errors.specification.condition = 'Укажите состояние!';
        ...
        if (values.landDetails) {
          if (!values.landDetails.landscapeKind) errors.landDetails.landscapeKind = 'Укажите тип участка!';
        ...
        if (values.communication) {
          if (!values.communication.powerSupply) errors.communication.powerSupply = 'Укажите значение!';
          if (!values.communication.waterSupply) errors.communication.waterSupply = 'Укажите тип водоснабжения!';
          if (!values.communication.gasSupply) errors.communication.gasSupply = 'Укажите тип газификации!';
          if (!values.communication.sewerageSupply) errors.communication.sewerageSupply = 'Укажите тип канализации!';
        } else {
  ...
  if (values.state === 'draft') {
    if (values.rentOffer) {
      if (!!values.rentOffer.period || !!values.rentOffer.deposit || !!values.rentOffer.isAgentFixed) {
  ...
  if (!values.complexBuildingId && values.complexBuildingDetails) {
    if (!values.complexBuildingDetails.houseKind) errors.complexBuildingDetails.houseKind = 'Обязательно';
    if (!values.complexBuildingDetails.builtYear) errors.complexBuildingDetails.builtYear = 'Обязательно';
    ...
    if (!values.complexBuildingDetails.constructionKind) errors.complexBuildingDetails.constructionKind = 'Обязательно';
    if (!values.complexBuildingDetails.elevators) errors.complexBuildingDetails.elevators = 'Обязательно';
