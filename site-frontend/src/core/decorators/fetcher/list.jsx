...
const pickState = ({ fetcher, pagination, filters }) => ({
  fetcher,
  pagination,
  filters,
});
...
export default settings => ChildListContainer =>
  connect(
    pickState,
    pickActions,
  )(
    class extends Component {
      ...
      componentWillReceiveProps(nextProps) {
        ...
        const nextPagination = nextProps.pagination[settings.id] || {
          offset: 0,
        };

        const isFilterUpdated =
          !isEqual(filter, nextFilter) ||
          !isEqual(fetcherFilter, nextFetcherFilter);
        const isFilterNotUpdated = !isEqual(
          fetcherFilterNot,
          nextFetcherFilterNot,
        );
        ...
        if (isFilterUpdated || isFilterNotUpdated || isPaginationUpdated) {
          const actionOptions = {
            queryParams: {
              ...
              pagination:
                isFilterUpdated || isFilterNotUpdated
                  ? { offset: 0 }
                  : nextPagination,
              orderBy: settings.orderBy,
            },
          ...
          this.props.fetcherActions.loadEntities(
            settings.id,
            actionOptions,
            settings.append && !isListResetNeeded,
          );
        }
      }
      ...
      getLinkedEntities() {
        return settings.linkedResourcesSchemes.reduce(
          (result, linkedEntity) => {
            ...
            const { list, ...linkedEntityItemsByIds } =
              resource[linkedEntity.typeId] || {}; // eslint-disable-line no-unused-vars
            return {
              ...result,
      ...
      render() {
        ...
        const { list: { items = [], isFetching = false, errors = [] } = {} } =
          fetcher[settings.id] || {};
        ...
        return (
          <section>
            <ChildListContainer
              ...
              errors={errors}
              linkedItemsMap={
                !!settings.linkedResourcesSchemes
              ...
              pagination={pagination[settings.id] || {}}
              filters={filters[settings.id] || {}}
            />
          </section>
        );
      }
