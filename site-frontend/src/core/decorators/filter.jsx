...
export default (kind, fields, extendedFields = [], options = {}) => OriginalComponent => connect(pickState, pickActions)(
  class extends Component {
    ...
    componentWillMount() {
      if (options.isMain) {
        this.setState({ storedValues: this.props.filters[kind || this.props.resourceName] || {} });
      }
    }
    ...
    generateOnChange(field) {
      ...
      return eventOrValue => {
        filtersActions.updateFilter(kind || this.props.resourceName, {
          [field]: isEvent(eventOrValue) ? eventOrValue.target.value : eventOrValue,
        });
    ...
    cancelSelectedFilter() {
      this.props.filtersActions.setFilter(kind || this.props.resourceName, this.state.storedValues);
    }
    ...
    render() {
      const values = this.props.filters[kind || this.props.resourceName] || {};
      const mappedFields = {};
      const filterCount = Object.keys(values).filter(key => this.valueIsNotEmpty(values[key])).length;
      const extendedFilterCount = Object.keys(values).filter(key => this.valueIsNotEmpty(values[key]) && (extendedFields.indexOf(key) > -1)).length;
