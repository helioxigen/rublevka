...
export default (fields, extendedFields = []) => (OriginalComponent) => connect(pickState, pickActions)(
  class extends Component {
    ...
    cancel() {
      this.props.dispatch(setFilter(this.props.resourceName, this.state.storedValues));
    }

    render() {
      const values = this.props.filters[this.props.resourceName] || {};

      const totalCount = Object.keys(values).filter(key => valueIsNotEmpty(values[key])).length;
      const extendedCount = Object.keys(values).filter(key => extendedFields.indexOf(key) > -1).filter(key => valueIsNotEmpty(values[key])).length;
