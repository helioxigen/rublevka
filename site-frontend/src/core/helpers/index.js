...
export const formatFilterDate = date => date ? `${moment(date).format(`YYYY-MM-DDTHH:mm:ss.SSS`)}Z` : undefined;
...
export const generateTimeSlotsList = () => Array(144).fill(0).map((element, index) => {
  const timeSlot = moment(`00:00`, `HH:mm`).add(index * 10, `minutes`).format(`HH:mm`);
  return {
    id: timeSlot,
    title: timeSlot,
  };
...
export const dateAndTimeToIso8601 = (date, time, utcOffset = 0) => moment(moment(date).format(`YYYY-MM-DD`) + `T` + moment(time, `HH:mm`).format(`HH:mm`)).utcOffset(utcOffset).format(`YYYY-MM-DDTHH:mm:ss.SSSZ`);

export const objectToQueryString = object => Object.keys(object).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(object[key])}`).join(`&`);
...
export const calculateTimeDiff = (timeFrom, timeTo = Date.now()) => {
  return !timeFrom ? 0 : moment(timeTo).diff(moment(timeFrom));
};
...
export const pluralize = (number: any, one: string, few: string, other: string) => {
  if (typeof number !== `undefined`) {
    if (number % 10 === 0 || (number >= 5 % 10 && number % 10 <= 9) || (number >= 11 % 100 && number % 100 <= 14)) return other;
...
const getChangeData = (change, previousModel, nextModel, keyPath, index) => {
  if (Array.isArray(change)) {
    ...
    if (change.length === 1) {
      const isArrayElement = parentKeyPathParts.length > 0 && parentKeyPathParts[parentKeyPathParts.length - 1].indexOf(`[`) > -1;
      return {
        type: `added`,
    ...
    if (change.length === 3 && change[2] === 3) return { type: `moved`, value: get(previousModel, keyPath), newIndex: change[1] };
  }
  return { type: `unknown` };
};

export const recursiveTraverseChanges = (object, previousModel, nextModel, currentKeyId = ``, currentKeyPath = ``, parentArrayIndex = null) => {
  const changes = [];

  Object.keys(object).forEach(key => {
    const [, arrayIndex = null] = key.match(/^_?(\d+)$/) || [];
    const newKeyPath = currentKeyPath ? `${currentKeyPath}${!!arrayIndex ? `[${arrayIndex}]` : `.${key}`}` : `${!!arrayIndex ? `[${arrayIndex}]` : key}`;
    const newKeyId = currentKeyId ? `${currentKeyId}${!!arrayIndex ? `` : `.${key}`}` : `${!!arrayIndex ? `` : key}`;

    if (object[key] && !Array.isArray(object[key]) && object[key] instanceof Object) {
      recursiveTraverseChanges(object[key], previousModel, nextModel, newKeyId, newKeyPath, arrayIndex).forEach(change => changes.push(change));
    } else if (key !== `_t`) {
...
export const formatPriceWithGrouping = (value) => {
  ...
  return `${numberParts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ` `)}${numberParts.length > 1 ? `.${numberParts[1]}` : ``}`;
};
...
export const formatByDictionary = (value, dictionary) => (
  dictionary[value]
);
...
export const getDateLimits = (date, kind, offset = 0, utcOffset = 0) => {
  if (kind === `start`) return `${moment(date).add(offset, `d`).hour(0).minute(0).second(0).millisecond(0).utcOffset(utcOffset).format(`YYYY-MM-DDTHH:mm:ss.SSS`)}Z`;
  if (kind === `end`) return `${moment(date).add(offset, `d`).hour(23).minute(59).second(59).millisecond(999).utcOffset(utcOffset).format(`YYYY-MM-DDTHH:mm:ss.SSS`)}Z`;
};
