...
import { getDefaultsByGroup, apiPathByGroup, resourceName } from '../../constants/defaults';
...
export const loadSettlementsByLetter = (queryParams, group, options) => (dispatch) => {
  dispatch(loadListStarted(types.LOAD_LIST, group));
  ...
  return loadList(apiPath, group, params).then(
    ({ items, pagination }) => {
      dispatch(updatePagination(`${resourceName}.${group}`, pagination));
      dispatch(loadListSucceeded(types.LOAD_LIST_BY_LETTER_SUCCEEDED, group, items));

      return Promise.resolve(items);
    },
    (errors) => {
      dispatch(loadListFailed(types.LOAD_LIST_BY_LETTER_FAILED, group, errors));
