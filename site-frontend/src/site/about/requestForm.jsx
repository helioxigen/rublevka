...
const { Button, Icon, Form, Grid, Image, Grid: { Row, Col }, Form: { Group, Input } } = UI;

import cn from 'classnames';
import s from 'site/styles/about/request';
import sUtils from 'site/styles/utils';

import { complexBuildingLeadRequestSettings as formSettings } from 'site/constants/leads/forms';
...
class Request extends Component {
  ...
  createLead() {
    const { actions, values: { ...values }, currentBroker } = this.props;
    const currentBrokerId = currentBroker.country.data ? currentBroker.country.data.staffUserId : 1;
  ...
  render() {
    ...
    return (
      <Form.Container onSubmit={handleSubmit(::this.createLead)} className={s.container}>
            ...
            <Row lg="center">
              {!!image && (
                <Col md="5" className={sUtils.hideXsSm}>
                  <Image
                    className={s.image}
                    src={`${global.config.cloudfront || cloudfront}/${image}-thumbnail-512`}
                    width="315"
                    height="315"
                    kind="circle"
                  />
              ...
              <Col md="7" className={s.textCenter}>
                ...
                <a href={`tel:+${global.config.phones.country}`} className={s.link}>
                ...
                <div className={cn(sUtils.pushedTopXs2_5Sm3, s.dividerTop)}>
                  <p className={cn(s.text, s.textBlack)}>Или оставьте номер и мы перезвоним</p>
                  <Row className={sUtils.pushedTop2_5}>
                    ...
                    <Col sm="4" lg="3">
                      <Button
                        className={cn(sUtils.pushedTopXs1_5Sm0, sUtils.borderRadius_4)}
                        kind="primary"
                        size="lg"
                        block
                      >
          ...
          <Grid.Container>
            <Row xs="center">
              <Col sm="10">
                ...
                <h2 className={cn(s.title, sUtils.pushedTop3)}>Заявка отправлена!</h2>
                <p className={cn(s.text, s.textGrey)}>Через 10 минут наш агент свяжется с Вами</p>
              </Col>
            </Row>
          </Grid.Container>
        )}
