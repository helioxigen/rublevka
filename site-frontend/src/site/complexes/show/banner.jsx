...
const {
  Icon, CountIndicator, Visibility,
  Grid: { Row, Col },
} = UI;

import cn from 'classnames';
import s from 'site/styles/complexes/id/banner';
...
class Banner extends Component {
  render() {
    ...
    return (
      <Row sm="center">
        <div className={s.banner} style={{ backgroundImage: publicImages.length ? `url(${global.config.cloudfront || cloudfront}/${publicImages[0].id}-${global.config.postfix}-1024)` : `url(${global.config.banner.image})` }} />
              ...
              <Col xs="12">
                <Visibility xs="hidden">
                  <nav className={sUtils.pushedTop3_5}>
                    ...
                    <Link to={'/gorodskaya/zhilye-kompleksy'} className={s.link}>
                      Жилые комплексы
                      {/* <Icon className={s.iconXs} icon="arrow-down" />*/}
                    </Link>
              ...
              <Col xs="12" className={sUtils.pushedTopXs5_2Sm6_4}>
                ...
                <div className={sUtils.pushedTop3}>
                  <h3 className={s.titleMd}>
                    {data.statistics &&
                      <CountIndicator count={data.statistics.primaryPropertiesCount} declensionForms={['предложение', 'предложения', 'предложений']} />
                    }&nbsp;
