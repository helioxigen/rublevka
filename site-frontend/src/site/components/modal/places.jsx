...
export default class extends Component {
  ...
  handleChange(value, toggle) {
    if (toggle) {
      ...
      const itemIndex = this.state.selectedItems.findIndex((settlement) => settlement.id === value.id);
      if (itemIndex >= 0) {
  ...
  render() {
    ...
    const buttonClassName = cn(s.btnModal, { [s.btnModalActive]: !!selectedItems.length });

    return (
      <div className={s.modal}>
        <Button className={buttonClassName} block size="md" kind="default" onClick={() => ::this.toggleModal(true)}>
          {!!selectedItems.length && selectedItems.slice(0, 4).map((item) => item.name).join(`, `) || `Поселок`}
        ...
        >
          ...
          <Grid.Container fluid className={s.container}>
            <Row>
              <Col xs="12">
                <h2 className={s.title}>Посёлки {!!preselectedItems.length && preselectedItems.length}</h2>
              </Col>
            </Row>
                ...
                <ul className={s.list}>
                  {preselectedItems.map((settlement) => (
                    <li key={settlement.id} className={s.listItem}>
                      {settlement.name}
                      <Button className={s.btnDelete} onClick={() => ::this.handleChange(settlement, false)}>
                  ...
                  <li className={s.listItem}>
                    {!!preselectedItems.length &&
                      <Button className={st.modal.btnDeleteAll} onClick={::this.reset}>Сбросить все</Button>
                    }
                  </li>
                </ul>
              </Col>
            ...
            <Row>
              <Col xs="12">
                <Form.Container className={s.pushed}>
                  <Input className={s.input} onChange={::this.searchSettlements} block placeholder="Введите название поселка" />
                </Form.Container>
              </Col>
            </Row>
            ...
            <Row className={s.fixHeight}>
              <Col sm="12">
                <div className={s.checkboxContainer}>
                  {items.map((settlement) =>
                    <Checkbox className={s.checkbox} key={settlement.id} handleChange={(ref, val) => ::this.handleChange(settlement, val)} reference={this.props.reference} checked={::this.isChecked(settlement.id)}>
                      <Text truncate={17} ellipsis title={settlement && settlement.name}>{settlement.name}</Text>
                    </Checkbox>
                  )}

                  {!items.length && !isFetching && (
                    <h2 className={s.titleLg}>Мы не нашли ничего по вашему запросу. <br />Попробуйте что-то изменить </h2>
                  )}
                </div>
              </Col>
              ...
              {pagination.total > items.length &&
                <Button className={s.btnDashed} onClick={::this.loadMore}>
                  <span className={s.borderDashed}>ещё {pagination.total - items.length} посёлков</span>
                </Button>
              }
            </Row>
          ...
          <Button className={s.btnModalAction} block kind="primary" size="xlg" onClick={::this.updateFilter}>Искать</Button>
        </Modal>
      </div>
    );
  }
