...
export default {
  list: {
    ...
    title: queryPage =>
      `Список элитных жилых комплексов Москвы | ${global.config.domain}${queryPage > 1
        ? ` — cтраница ${queryPage}`
        : ''}`,
    description: `Лучшие жилые комплексы Москвы | ${global.config.domain}`,
    keywords: `список ЖК, жилые комплексы, Москва, аренда, продажа, купить, снять, ${global.config
      .domain}`,
  ...
  show: {
    ...
    title: (name, street) => {
      ...
      return `ЖК ${name} — аренда и продажа квартир в жилом комплексе ${name}${street
        ? ` — ${street}`
    ...
    description: (name, street) => {
      ...
      return `Купить или снять квартиру в ЖК ${name}, расположенного в Москве${street
        ? ` на ${street}`
