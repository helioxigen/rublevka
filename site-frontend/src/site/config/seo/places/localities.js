...
export default {
  list: {
    ...
    title: `Найти недвижимость в Московской области по населенному пункту – ${global.config
      .domain}`,
    description: `Список населенных пунктов для поиска недвижимости в Подмосковье на ${global.config
      .domain}`,
    keywords: `населенные пункты, загородная недвижимость, список, перечень, Московская область, Подмосковье, ${global
      .config.domain}`,
  },
  show: {
    h1: ({ name }, dealType, kind) => {
      ...
      return `${dictionary[dealType].title} ${dictionary[dealType].kind[
        kind
      ]} – нас. пункт ${name}`;
    },
    title: ({ name }, dealType, kind) => {
      ...
      if (!kind) {
        return `${dictionary[dealType]
          .title} загородной недвижимости – ${name}, Московская область – ${global.config.domain}`;
      }
