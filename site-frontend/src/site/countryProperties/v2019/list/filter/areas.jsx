...
import { areas, landAreas } from 'site/constants/leads/options';
      ...
      <section>
        ...
        <Container fluid styleName="contentContainer">
          <FilterHeader>
            <Title>
              Площадь&nbsp;
              {(kind.length === 0 || kind.includes('house') || kind.includes('townhouse')) ? 'дома' : 'квартиры'}
              &nbsp;(м²)
             </Title>
            {Object.keys(area).length > 0 && <IconReset onClick={() => this.onReset('area')} icon="times" />}
          </FilterHeader>
          <ControlsContainer>
            <SelectGroup
              options={areas}
        ...
        <Container fluid styleName="contentContainer">
          <FilterHeader>
            ...
            {Object.keys(landArea).length > 0 && <IconReset onClick={() => this.onReset('landArea')} icon="times" />}
          </FilterHeader>
          <ControlsContainer>
            <SelectGroup
              options={landAreas}
