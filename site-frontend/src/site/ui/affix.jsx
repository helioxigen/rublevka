...
import s from 'site/styles/ui/affix';
...
export class AffixBottom extends Component {
  ...
  render() {
    const { children, height, position = `bottom`, className: parentClassName, ...props } = this.props; // eslint-disable-line no-unused-vars
    const className = cn({ [s.affixBottom]: this.state.affix }, children.props.className);
    const style = { height: `${height}px` };

    return (
      <div ref="parent" style={style}>
        {React.cloneElement(children, { ...children.props, ...props, className: className })}
      </div>
    );
  }
}
