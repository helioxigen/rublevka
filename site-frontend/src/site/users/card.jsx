      ...
      return (
        <Col sm="6" md="4" className={s.cardContainer}>
          ...
          <div className={s.cardAgent}>
            <Image className={s.image} src={`${global.config.cloudfront || cloudfront}/${photo.id}-512`} kind="circle" />
            ...
            <div className={sUtils.pushedTopXs2_5Sm4}>
              ...
              {showExperience && details.experienceYears && (
                <h4 className={cn(s.titleMd, sUtils.pushedTopXs3Sm2)}>
                  Опыт — <CountIndicator count={details.experienceYears} declensionForms={[`год`, `года`, `лет`]} />
                </h4>
              )}
          ...
          <Modal contentClassName={s.modal} onClose={() => ::this.toggleModal(false)} isOpened={this.state.modalOpen} closeOnEsc closePortal={() => ::this.toggleModal(false)} closeOnOutsideClick>
            <div className={s.modalContainer}>
              {data.photo && <Image className={s.image} src={`${global.config.cloudfront || cloudfront}/${photo.id}-512`} width="204" height="204" kind="circle" />}
              ...
              <div className={s.descriptionContainer}>
                ...
                {details.education &&
                  <div className={sUtils.pushedTop4}>
                    ...
                    <p className={cn(s.text, sUtils.pushedTop2, s.resetPadding)}>{details.education}</p>
                  </div>
                }
                {details.description &&
                  <div className={sUtils.pushedTop4}>
                    <h3 className={s.titleMd}>Опыт — <CountIndicator count={details.experienceYears} declensionForms={[`год`, `года`, `лет`]} /></h3>
                    <ReactMarkdown className={cn(s.text, s.resetPadding, s.paddingBottomSm8)} source={details.description} />
                  </div>
                }
              </div>
            </div>
